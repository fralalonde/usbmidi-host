...
impl Device {
    ...
    pub async fn get_device_descriptor(&mut self, host: &mut dyn USBHost) -> Result<DeviceDescriptor, TransferError> {
        let mut dev_desc: DeviceDescriptor = DeviceDescriptor::default();
        self.control_get_descriptor(host, DescriptorType::Device, 0, to_slice_mut(&mut dev_desc)).await?;
    ...
    pub async fn get_configuration_descriptors(&mut self, host: &mut dyn USBHost, cfg_idx: u8, buffer: &mut [u8]) -> Result<usize, TransferError> {
        let mut config_root: ConfigurationDescriptor = ConfigurationDescriptor::default();
        self.control_get_descriptor(host, DescriptorType::Configuration, cfg_idx, to_slice_mut(&mut config_root)).await?;
        if config_root.w_total_length as usize > buffer.len() {
            Err(TransferError::Permanent("Device config larger than buffer"))
        } else {
            self.control_get_descriptor(host, DescriptorType::Configuration, cfg_idx, buffer).await
        }
    }
    ...
    pub async fn set_address(&mut self, host: &mut dyn USBHost, dev_addr: Address) -> Result<(), TransferError> {
    ...
    pub async fn set_configuration(&mut self, host: &mut dyn USBHost, config_num: u8) -> Result<(), TransferError> {
        if config_num == 0 {
            return Err(TransferError::Permanent("Invalid device configuration number"));
        }
        self.control_set(host, RequestCode::SetConfiguration, config_num, 0, 0).await
    }

    // TODO get_interface()
    pub async fn set_interface(&mut self, host: &mut dyn USBHost, iface_num: u8, alt_num: u8) -> Result<(), TransferError> {
        self.control_set(host, RequestCode::SetInterface, alt_num, 0, iface_num as u16).await
    }
}
...
impl ControlEndpoint for Device {
    ...
    async fn control_get_descriptor(&self, host: &mut dyn USBHost, desc_type: DescriptorType, desc_index: u8, buffer: &mut [u8]) -> Result<usize, TransferError> {
